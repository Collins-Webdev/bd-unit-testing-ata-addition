...
public class AtaAdditionTest {
    ...
    @Test
    public void add_sumUnderflow_throwsArithmeticException() {
        // GIVEN
        int[] values = {Integer.MIN_VALUE + 5, -3, -3};
        AtaAddition ataAddition = new AtaAddition();

        // WHEN + THEN
        // asserting that when the inner code block calls the add() method we should see an `ArithmeticException`
        Assertions.assertThrows(ArithmeticException.class, () -> {
            ataAddition.add(values);
